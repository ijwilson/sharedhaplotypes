...
```
tabix -fh ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/phase1/analysis_results/integrated_call_sets/ALL.chr2.integrated_phase1_v3.20101123.snps_indels_svs.genotypes.vcf.gz 2:178404570-179734924 > tmpfiles/temporary.vcf
```

#### transform to get phased output in IMPUTE format

```
vcftools --phased --vcf temporary.vcf --IMPUTE --out impute1K 
rm temporary.vcf 
```
...
```
we have read `r nrow(positions_1K)` positions from the 1000 genomes.

Use python to stip out the usable SNPs from the 1000 genomes.
